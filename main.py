import cv2
import mediapipe as mp

import pyautogui

x1 =y1 = x2 = y2= 0
webcam  = cv2.VideoCapture(0)
my_hands = mp.solutions.hands.Hands()
drawing_utils = mp.solutions.drawing_utils

while True:
    _ , image = webcam.read()
    image = cv2.flip(image,1)
    fragment_height,fragment_width,_ = image.shape
    rgb_image =cv2.cvtColor(image,cv2.COLOR_BGR2RGB)
    output = my_hands.process(rgb_image)
    hands = output.multi_hand_landmarks
    if hands:
        for hand in hands :
            drawing_utils.draw_landmarks(image,hand)
            landmarks = hand.landmark
            for id, landmark in enumerate(landmarks):
                x= int (landmark.x * fragment_width)
                y= int (landmark.y * fragment_height)
                if id ==8:
                    cv2.circle(img=image,center =(x,y),radius=8,color=(255,255,255),thickness=3)
                    x1= x
                    y1= y
                if id ==4:
                    cv2.circle(img=image,center =(x,y),radius=8,color=(255,0,255),thickness=3)
                    x2= x
                    y2= y

        dist  = (((x2 - x1) ** 2 + (y2 - y1) ** 2) ** 0.5 )// 4

        cv2.line(image,(x1,y1),(x2,y2),(0,255,255),5)
        if dist > 40:  # Increase volume
            print("Volume Up")
            pyautogui.press("volumeup")
        elif dist < 30:  # Decrease volume
            print("Volume Down")
            pyautogui.press("volumedown")

    cv2.imshow("Hand Volume using Python",image)

    key = cv2.waitKey(5)
    if key == 27:
        break

webcam.release()
cv2.destroyAllWindows()





